...
if __name__ == "__main__":
    ...
    parser.add_argument("--continues", type=int, default=0, help="Defines log begin with which epoch")
...
#     parser.add_argument("--continues", type=int, default=0, help="Defines log begin with which epoch")
    ...
    os.makedirs("checkpoints_coco", exist_ok=True)
    ...
    for epoch in range(opt.epochs):
        ...
        if epoch % opt.evaluation_interval == 0:
            ...
            if metrics_output is not None:
                ...
                print(f"---- mAP {AP.mean()}")                
                log_file = open("coco_maps.txt", "a+")
                log_file.write("mAP:" + str(AP.mean()) + ", checkpoints_coco/yolov3_ckpt_%d.pth" % (epoch + begin_number) + "\n")
                log_file.close()
                remember = False
                if AP.mean() > best_map:
                    best_map = AP.mean()
                    remember = True
                    best_file = open("best_map.txt", "w")
                    best_file.write("mAP:" + str(AP.mean()) + ", checkpoints_coco/yolov3_ckpt_%d.pth" % (epoch + begin_number))
                    best_file.close()
            else:
                print( "---- mAP not measured (no detections found by model)")

        if epoch % opt.checkpoint_interval == 0 and remember:
            torch.save(model.state_dict(), f"checkpoints_coco/yolov3_ckpt_%d.pth" % (epoch + begin_number) )
